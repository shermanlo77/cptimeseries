...
class Arma(object):
    ...
    Evaluates ARMA terms. To be owned by instances of Parameter so that their
        methods ar(), ma(), d_reg_ar() and d_reg_ma() can be called here. AR and
        MA terms at the start of the time series are evaluated to be zero.
    ...
    def d_reg_ar_term(self, index, key):
        ...
        Return the derivative of the AR term \sum_{i=1}^p\phi_{i}\Phi(t-i) with
            respect to a regression parameter (eg \beta, \phi, \theta, k). The
            return value is a vector of the same shape as the regression
            parameter
    ...
    def d_reg_ma_term(self, index, key):
        ...
        Return the derivative of the MA term \sum_{i=1}^q\theta_{i}\Theta(t-i)
            with respect to a regression parameter (eg \beta, \phi, \theta, k).
            The return value is a vector of the same shape as the regression
            parameter
