"""Implementations of MCMC algorithms targetting continuous target
    distributions
...
class Rwmh(mcmc_abstract.Mcmc):
    ...
    def update_chain_statistics(self):
        ...
        # online update the covariance matrix
            ...
            self.chain_covariance += ((n/math.pow(n-1, 2))
                                      * np.outer(diff, diff))
...
class Elliptical(mcmc_abstract.Mcmc):
    ...
    def sample(self):
        ...
        while is_sampling:
            ...
            # change the search space if the new proposed parameter was
            # rejected
            if is_accept:
                is_sampling = False
            else:
