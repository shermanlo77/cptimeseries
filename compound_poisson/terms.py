"""Classes representing terms in a compound Poisson series

Classes representing terms in a compound Poisson series, usually denoted as W
    in Dunn, Smyth (2005). Used to work out sums or conditional expectations of
    Z and Z^2 in the EM algorithm.
"""
...
class Terms(object):
    ...
    Attributes:
        y: realisation of the compound-Poisson
        poisson_rate: float, positive, parameter for the compound-Poisson
        gamma_mean: float, positive, parameter for the compound-Poisson
        gamma_dispersion: float, positive, parameter for the compound-Poisson
    """
    ...
    def __init__(self, parent, index):
        """
        Args:
            parent: TimeSeries object which owns this object
            index: integer, time index to consider
        """
        self.y = parent[index]
    ...
    def ln_sum_w(self):
        ...
        while not is_got_z_l:
            ...
            if z_l == 0:
                ...
                log_ratio = np.sum(
                    [self.ln_wz(z_l),
                     self.log_expectation_term(z_l), -ln_w_max])
