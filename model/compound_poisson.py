...
parameter_0 = (0.098, 0.713, 0.11, 0.355) #poisson
parameter_1 = (0.066, 0.40, 0.24, 1.3) #mean
...
for i in range(1,n):
    ...
    poisson_rate_array[i] = math.exp(parameter_0[0] * x[i]
        + parameter_0[1] * (math.log(poisson_rate_array[i-1]) - parameter_0[3])
        + parameter_0[2] * (z[i-1] - poisson_rate_array[i-1])
            / math.sqrt(poisson_rate_array[i-1])
        + parameter_0[3])
    
    exponent = parameter_1[0] * x[i] \
        + parameter_1[1] *  (math.log(mean_gamma_array[i-1]) - parameter_1[3]) \
        + parameter_1[3]
    if z[i-1] > 0:
        exponent +=\
            parameter_1[2] * (y[i-1]/z[i-1] - mean_gamma_array[i-1]) \
                / mean_gamma_array[i-1] \
                / math.sqrt(dispersion_array[i-1]*z[i-1])
