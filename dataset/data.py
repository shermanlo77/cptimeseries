...
class Data(object):
    ...
    Attributes:
        ...
        time_array: array of datetime objects
    ...
    def load_model_field(self, file_name):
        """Load model fields from netCDF4 file

        Load model fields onto memory. Only the file ana_input_1.nc is supported
            at the moment.

        Notes:
            ana_input_1.nc contains the interpolate model fields
            The grid from ana_input_1.nc is slightly bigger than the fine
                grid so require cropping

        Args:
            file_name: name of the file to read from, only ana_input_1.nc
                supported
        """
        ...
        for key, model_field in dataset.items():
            #dataset contains time and coordinates
            #interested in spatial temporal data which has 3 dimensions
            if len(model_field.shape) == 3:
                #check the dimension names
                assert(model_field.dimensions[1] == "latitude")
                assert(model_field.dimensions[2] == "longitude")
                ...
                new_keys.append(model_field_name)
        ...
        dataset_generator = GeneratorNc(
            dataset, dataset_key, 4, time_array)
        model_field = self.get_derive_smooth_model_field(dataset_generator,
                                                         new_keys,
                                                         latitude_array,
                                                         longitude_array)
        dataset_file.close()
        ...
        for key, value in model_field.items():
            model_field[key] = np.asarray(value)[
                :, latitude_min:latitude_max, longitude_min:longitude_max]
            assert(model_field[key][0].shape == target_shape)
...
class DataDualGrid(Data):
    ...
    Attributes:
        model_field_coarse: dictionary, keys are name of model fields, values
            ...
            grid
        topography_coarse: dictionary, keys are names of topography, values are
            a matrix with topography values, corresponding on the coarse grid
        topography_coarse_normalise: dictionary, keys are names of topography,
            values are a matrix with normalised topography values, corresponding
            on the coarse grid. Normalised beforehand on the fine grid
    """
    ...
    def copy_from(self, other):
        """Shallow copy from another Data object
        """
    ...
    def load_topo(self, file_name):
        """Load topography data, both on fine and coarse grid, into memory

        Coarse and fine grid do not overlap exactly, interpolate topography from
    ...
    def load_model_field_interpolate_to_coarse(self, file_name):
        """Load model fields on fine grid and get coarse grid model fields by
        interpolating from the fine grid

        To be used for .nc files
        """
        super().load_model_field(file_name)
        self.model_field = self.model_field.copy()
