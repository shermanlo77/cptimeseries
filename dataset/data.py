...
class DataDualGrid(object):
    ...
    Attributes:
        ...
        model_field_coarse: dictionary, keys are name of model fields, values
            ...
            grid
        model_field_units: dictionary, keys are name of model fields, values
            are strings with represent the units the model fields are measured
            in
        ...
        topography_normalise: dictionary, keys are names of topography, values
            are a matrix with normalised topography values, corresponding on
            the fine grid. Normalised to have mean 0, std 1
    """
    ...
    def load_model_field(self, file_name):
        ...
        for i in range(raster_array.RasterCount):
            ...
            if grib_comment not in grib_comment_array:
        ...
        for grib_comment in grib_comment_array:
            ...
            if model_field_name not in self.model_field_units:
                self.model_field_units[model_field_name] = units

        # smooth daily readings and get derived model fields, eg using
        # gradients
        raster_generator = GeneratorGrib(
            raster_array, raster_pointer, 4, time_array)
        model_field = self.get_derive_smooth_model_field(
            raster_generator, model_field_name_array,
            LATITUDE_COARSE_ARRAY, LONGITUDE_COARSE_ARRAY)
    ...
    def get_derive_smooth_model_field(self,
        ...
        for i in range(0, len(time_array), n_read_per_day):
            ...
            for model_field_name in model_field_smooth:
                ...
                if model_field_name not in DERIVED_MODEL_FIELD:
    ...
    def normalise_topography(self):
        """Store the normalisation of self.topography, to have mean 0, std 1,
            in self.topography_normalise
        """
        self.topography_normalise = {}
    ...
    def find_nearest_latitude_longitude(self, coordinates):
        ...
        Given coordinates of a place, returns the nearest latitude and
            longitude for a specific grid
    ...
    def get_latitude_longitude_random_mask(self, rng):
        mask_index = np.where(np.logical_not(self.mask))
    ...
    def get_data_city(self, city):
        latitude_index, longitude_index = self.get_latitude_longitude_city(
            city)
